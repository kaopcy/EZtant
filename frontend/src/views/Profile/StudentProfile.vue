<template>
    <div class="main-wrapper">
        ...
        <div class="posts" v-if=" selectedTab === 'post'">
            <Post :userID="user.id" :postData="requestedPost" />
        </div>
...
export default {
    ...
    setup() {
        ...
        const { requestedPost , getRequestedPostbyID } = usePost()

        onMounted( async ()=>{
            await getRequestedPostbyID(user.value.id)
        })

        provide('user' , user)
        
        return { user, selectedTab , requestedPost };
    },
};
</script>
